          ...
          <Actionsheet isOpen={isOpen} onClose={onClose}>
            <Actionsheet.Content>
              <Box alignSelf="flex-start" ml={3}>
                <Text fontSize="md" fontWeight="medium">Schedule</Text>
                <Text fontSize="md" color="muted.500">{schedule.occurrence} on {schedule.day}</Text>
              </Box>
              <HStack space={3} m={3}> 
                <Button variant="subtle" rounded="3xl" w="25%" _text={{ color: 'primary.600', fontWeight: 'semibold', mb: '0.5' }}
                onPress={()=>setSchedule({day: "every", occurrence: "Daily"})}>Daily</Button>
                <Button variant="subtle" rounded="3xl" w="25%" _text={{ color: 'primary.600', fontWeight: 'semibold', mb: '0.5' }}
                onPress={()=>setSchedule({day: schedule.day, occurrence: "Weekly"})}>Weekly</Button>
                <Button variant="subtle" rounded="3xl" w="25%" _text={{ color: 'primary.600', fontWeight: 'semibold', mb: '0.5' }}
                onPress={()=>setSchedule({day: schedule.day, occurrence: "Monthly"})}>Monthly</Button>
              </HStack>
              <Box maxH={120} w="2/3" my={2}>
                ...
                renderItem={({item})=>(
                  <TouchableOpacity onPress={()=>setSchedule({day: item, occurrence: schedule.occurrence})}>
                    <Text fontSize="xl" color="primary.600" py={1} textAlign="center" borderBottomWidth={1}  borderBottomColor="muted.500">{item}</Text>
                  </TouchableOpacity>
                )}/>
                ):(
                  <>
                    <Text fontWeight="medium" textAlign="center">Its Gonna be Daily.</Text>
                    <Text textAlign="center" color="primary.600">
                      This frequency means that your first contribution will start Today
                      and repeat everyday after that.
                    </Text>
                  </>
                )}
                
              </Box>
              <Button variant="subtle" rounded="3xl" w="60%" my={3} _text={{ color: 'primary.600', fontWeight: 'semibold', mb: '0.5' }}
               onPress={()=>{
                if(isSetCtb){
