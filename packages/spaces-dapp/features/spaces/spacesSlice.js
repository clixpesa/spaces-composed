...
export const getRosca = createAsyncThunk(
  'spaces/getRosca',
...
export const getRoscaAddress = createAsyncThunk(
  'spaces/getRoscaAddress',
  async (spacesContract, thunkAPI) => {
    try {
  ...
  reducers: {
    ...
    setGoalAmount: (state, { payload }) => {
      ...
      state.spaceInfo.ctbAmount = size
        ? payload / state.spaceInfo.members.length
        : payload;
    },
